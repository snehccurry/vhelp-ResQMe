...
def main():
    ...
    sub = Popen(["pyinstaller", "--clean", "vhelpinstaller.spec"], env=env)
    sub.communicate()
    ...
    vhelp_dir = os.path.join(curdir, "dist", "vhelp")
    executable = os.path.join(vhelp_dir, "vhelp"+EXE_EXT)
    ...
    # run the result binary using "cmd.exe /k vhelp.exe", so that
    ...
    if platform.system() == "Windows":
        if "--debug" in sys.argv:
            ...
            os.system("%s/explorer.exe /n,/e,%s" % (
                os.environ["SYSTEMROOT"], vhelp_dir))
